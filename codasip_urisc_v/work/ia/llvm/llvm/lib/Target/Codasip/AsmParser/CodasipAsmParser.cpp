...
extern int yyparse(yyscan_t scanner, llvm::MCAsmParser &parser,
...
cl::opt<bool>
    EnableAsmCompression("enable-asm-compression", cl::Hidden, cl::init(true),
...
class CodasipAsmParser : public MCTargetAsmParser {
  ...
  static bool Tokens2Operands(OperandVector &Operands,
...
bool CodasipAsmParser::ParseInstruction(ParseInstructionInfo &Info,
  ...
  if (Tokens2Operands(Operands, m_ParsedTokens)) {
      return true;
  }
 ...
 */
bool CodasipAsmParser::Parse(codasip::ast::assembler::Node *&Ast,
                             llvm::SMLoc NameLoc) {
  yyscan_t Scanner;
  yylex_init(&Scanner);
...
bool CodasipAsmParser::Tokens2Operands(
    OperandVector &Operands,
    codasip::assembler::ParserUtility::ParsedTokens &tokens) {
  for (codasip::assembler::ParsedToken *Token : tokens) {
    ...
    if (Token->GetType() == codasip::assembler::ParsedToken::TOKEN) {
      ...
      if (Token->GetString() == ",")
        continue;
      Operands.emplace_back(CodasipOperand::createToken(
          StringRef(Token->GetString().c_str(), Token->GetString().size()), S));
      ...
      codasip::assembler::ParserUtility::ConvertCodasipToLlvmTokens(
          llvmTokens, Token->GetParsedTokens());
...
bool CodasipAsmParser::ParseDirectiveProfiler() {
  ...
  if (Flags == "s")
    ...
    return Error(S, "Unsupported '.profiler' directive flag '" + Flags +
                        "'. The only supported '.profiler' directive flags are "
                        "'s' (start) and 'x' (end)");
