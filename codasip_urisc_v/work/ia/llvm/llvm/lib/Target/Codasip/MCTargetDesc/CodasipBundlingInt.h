...
// Forward declaration.
template <int BITS> class CodasipBundlingInt;
...
struct numeric_limits<llvm::CodasipBundlingInt<_CODASIP_BITS>>
    : public ::codasip::integer::detail::NumericLimitsBase<
          llvm::CodasipBundlingInt<_CODASIP_BITS>> {};
...
public:
  ...
  CodasipBundlingBitInfo(char value, unsigned int index)
      : m_Value(value), m_Index(index), m_Slot(UNDEFINED_SLOT),
        m_OriginIndex(UNDEFINED_INDEX), m_OriginFixupIndex(UNDEFINED_INDEX),
        m_OriginFixupBitIndex(UNDEFINED_INDEX) {}

  CodasipBundlingBitInfo(char value, unsigned int index, int slot,
                         int originIndex, int originFixupIndex,
                         int originFixupBitIndex)
      : m_Value(value), m_Index(index), m_Slot(slot),
        m_OriginIndex(originIndex), m_OriginFixupIndex(originFixupIndex),
        m_OriginFixupBitIndex(originFixupBitIndex) {}

  CodasipBundlingBitInfo(const CodasipBundlingBitInfo &copy)
      : m_Value(copy.GetValue()), m_Index(copy.GetIndex()),
        m_Slot(copy.GetSlot()), m_OriginIndex(copy.GetOriginIndex()),
  ...
  int GetOriginIndex() const { return m_OriginIndex; }

  int GetOriginFixupIndex() const { return m_OriginFixupIndex; }

  int GetOriginFixupBitIndex() const { return m_OriginFixupBitIndex; }

  void SetIndex(const unsigned int index) { m_Index = index; }

  void SetOriginFixupInfo(const int originFixupIndex,
                          const int originFixupBitIndex) {
    if (m_OriginFixupIndex != UNDEFINED_INDEX ||
        m_OriginFixupBitIndex != UNDEFINED_INDEX) {
  ...
  void operator&=(const CodasipBundlingBitInfo &value) {
    if ((GetValue() == ONE_BIT || GetValue() == ZERO_BIT) == false ||
        (value.GetValue() == ONE_BIT || value.GetValue() == ZERO_BIT) ==
            false) {
      std::cerr << "Cannot negate bit with value: " << GetValue()
                << " with value: " << value.GetValue() << std::endl;
  ...
  void Print() const {
    ...
    if (GetSlot() != UNDEFINED_SLOT) {
      std::cout << " is representing slots: " << GetSlot() << " index: ["
                << GetOriginIndex() << "]";
...
private:
  ...
  int m_Slot;
  /// index of the bit in the origin bit field of CodasipBundlingInt which
  /// stored original instruction
  int m_OriginIndex;
 ...
 *         after bundling operation we get result which has information about
 * which bit is taken from which slot and index
 */
template <int BITS> class CodasipBundlingInt {
private:
...
public:
  enum { BIT_WIDTH = BITS, IS_SIGNED = false };

  template <int R_BITS> struct return_type {
    enum { BIT_W = __CODASIP_MAX(BIT_WIDTH, R_BITS) };

    typedef CodasipBundlingInt<BIT_W> bit_t;
  };

  // Helper methods
  const CodasipBundlingBitInfo::Content &GetContent() const { return m_Bits; }
  ...
  void CopyContent(const CodasipBundlingInt<R_BITS> &source) {
    ...
    if (BIT_WIDTH > R_BITS) {
      ...
      for (int i = 0; i < bitWidth; i++) {
        m_Bits.push_back(CodasipBundlingBitInfo(
            CodasipBundlingBitInfo::ZERO_BIT, ComputeIndex(i),
            CodasipBundlingBitInfo::UNDEFINED_SLOT,
            CodasipBundlingBitInfo::UNDEFINED_INDEX,
            CodasipBundlingBitInfo::UNDEFINED_INDEX,
            CodasipBundlingBitInfo::UNDEFINED_INDEX));
      }
    }
    ...
    for (int i = startIndex; i < source.BIT_WIDTH; i++) {
      m_Bits.push_back(CodasipBundlingBitInfo(
          bits[i].GetValue(), ComputeIndex(m_Bits.size()), bits[i].GetSlot(),
          bits[i].GetOriginIndex(), bits[i].GetOriginFixupIndex(),
          bits[i].GetOriginFixupBitIndex()));
    }
  }

  /**
   *  \brief Sets valid indexes of CodasipBundlingBitInfos after shifts for
   * example
   */
  void ResetIndexes() {
  ...
  // Constructors
  CodasipBundlingInt() : m_Slot(CodasipBundlingBitInfo::UNDEFINED_SLOT) {
    DEBUG_OUT("Common constructor");
  }

  CodasipBundlingInt(const std::string &source, int slot) : m_Slot(slot) {
    ...
    for (size_t i = 0; i < source.size(); i++) {
      ...
      m_Bits.push_back(
          CodasipBundlingBitInfo(source[i], index, GetSlot(), index,
                                 CodasipBundlingBitInfo::UNDEFINED_INDEX,
                                 CodasipBundlingBitInfo::UNDEFINED_INDEX));
    }
  }
  ...
  CodasipBundlingInt(const codasip::Integer<R_BITS, SIGNED> &value)
    ...
    std::string binary = value.GetBin(false, false);
  ...
  void SetFixupBit(int index, int originFixupIndex, int originFixupBitIndex) {
    m_Bits[ComputeIndex(index)].SetOriginFixupInfo(originFixupIndex,
                                                   originFixupBitIndex);
  }
  ...
  template <typename T> CodasipBundlingInt &operator=(const T r) {
    return (*this) = CodasipBundlingInt(r);
  }

/**
 *  \brief Creates conversion constructor to create CodasipBundlingInt from
 * given integer data type.
 */
#define __CODASIP_BUNDLING_INT_CONVERSION_OPERATOR_WITH_INT_TYPE(ctype)        \
  CodasipBundlingInt(const ctype value)                                        \
    ...
    std::string binary = local.GetBin(false, false);                          \
    for (int i = 0; i < BIT_WIDTH; i++) {                                      \
      m_Bits.push_back(CodasipBundlingBitInfo(binary[i], ComputeIndex(i)));    \
    }                                                                          \
  }
  ...
  __CODASIP_BUNDLING_INT_CONVERSION_OPERATOR_WITH_INT_TYPE(
  ...
  bool equal_to(const CodasipBundlingInt<R_BITS> &r) const {
    ...
    size_t left = 0;
    size_t right = 0;
    if (static_cast<unsigned int>(BIT_WIDTH) >
        static_cast<unsigned int>(r.BIT_WIDTH)) {
      for (left = 0; left < (static_cast<unsigned int>(BIT_WIDTH) -
                             static_cast<unsigned int>(r.BIT_WIDTH));
           left++) {
      ...
      for (right = 0; right < (static_cast<unsigned int>(r.BIT_WIDTH) -
                               static_cast<unsigned int>(BIT_WIDTH));
           right++) {
        if (r.GetContent()[right].GetValue() !=
            CodasipBundlingBitInfo::ZERO_BIT) {
          return false;
        }
      }
    }
  ...
  CodasipBundlingInt &operator&=(const CodasipBundlingInt<R_BITS> &r) {
    ...
    if ((unsigned)BIT_WIDTH > (unsigned)r.BIT_WIDTH) {
      for (left = 0; left < (BIT_WIDTH - r.BIT_WIDTH); left++) {
        m_Bits[left] &= CodasipBundlingBitInfo(CodasipBundlingBitInfo::ZERO_BIT,
                                               m_Bits[left].GetIndex());
      }
    } else {
  ...
  CodasipBundlingInt &operator<<=(const unsigned value) {
    if (value > BIT_WIDTH) {
      ...
      oss << __FILE__ << "(" << __LINE__
          << ") error: too large shift count: " << value;
      throw codasip::exceptions::BasicError(oss.str());
    }
    ...
    for (unsigned i = 0; i < value; i++) {
      ...
      m_Bits.push_back(
          CodasipBundlingBitInfo(CodasipBundlingBitInfo::ZERO_BIT, 0));
    }
  ...
  CodasipBundlingInt &operator>>=(const unsigned value) {
    ...
    for (unsigned i = 0; i < value; i++) {
      CHECK_NOT_EMPTY(m_Bits);
      m_Bits.pop_back();
      m_Bits.insert(m_Bits.begin(), CodasipBundlingBitInfo(
                                        CodasipBundlingBitInfo::ZERO_BIT, 0));
    }
  ...
  template <typename T> CodasipBundlingInt operator<<(const T r) const {
   ...
   */
  void Print() const { CodasipBundlingBitInfo::PrintContent(m_Bits); }
};

#ifndef __TYPE_WIDTH
#define __TYPE_WIDTH(T)                                                        \
...
/**
 *  \brief Defines all forms of comparison operator. First version defines
 * comparison of two CodasipBundlingInt objects. Other versions use
 * CodasipBundlingInt as one operand and the other should be any standard
 * integer type.
 */
...
/**
 *  \brief Defines template binary operator in three forms. First version
 * defines operation with CodasipBundlingInt as left and right operands. Other
 * versions use CodasipBundlingInt as one operand and the other should be any
...
#define __CODASIP_BUNDLING_INT_BINARY_OPERATOR(OP, RET_TYPE)                   \
  ...
  operator OP(const CodasipBundlingInt<L_BITS> &lhs,                           \
              const CodasipBundlingInt<R_BITS> &rhs) {                         \
    typedef typename CodasipBundlingInt<L_BITS>::template return_type<         \
        R_BITS>::RET_TYPE ret_type;                                            \
    ret_type nrv(lhs);                                                         \
    nrv OP## = rhs;                                                            \
    return nrv;                                                                \
  }                                                                            \
  template <typename T, int L_BITS>                                            \
  typename CodasipBundlingInt<L_BITS>::template return_type<__TYPE_WIDTH(      \
      T)>::RET_TYPE                                                            \
  operator OP(const CodasipBundlingInt<L_BITS> &lhs, const T rhs) {            \
    typedef CodasipBundlingInt<__TYPE_WIDTH(T)> rhs_type;                      \
    typedef typename CodasipBundlingInt<L_BITS>::template return_type<         \
        rhs_type::BIT_WIDTH>::RET_TYPE ret_type;                               \
    ret_type nrv(lhs);                                                         \
    nrv OP## = rhs_type(rhs);                                                  \
    return nrv;                                                                \
  }                                                                            \
  template <typename T, int R_BITS>                                            \
  typename CodasipBundlingInt<__TYPE_WIDTH(                                    \
      T)>::template return_type<R_BITS>::RET_TYPE                              \
  operator OP(const T lhs, const CodasipBundlingInt<R_BITS> &rhs) {            \
    typedef typename CodasipBundlingInt<__TYPE_WIDTH(                          \
        T)>::template return_type<R_BITS>::RET_TYPE ret_type;                  \
    ret_type nrv(lhs);                                                         \
    nrv OP## = rhs;                                                            \
    return nrv;                                                                \
  }
