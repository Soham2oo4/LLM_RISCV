...
llvm::CodasipInstrInfo::CodasipInstrInfo(CodasipTargetMachine &TM)
    : CodasipGenInstrInfo2(TM) {}
...
void llvm::CodasipInstrInfo::storeRegToStackSlot(
  ...
  CodasipGenInstrInfo2::storeRegToStackSlot(MB, II, Reg, isKill, FrameIndex, RC,
                                            TRI);
}
...
bool llvm::CodasipInstrInfo::ExpandDummiesImpl(MachineInstr *MIp,
                                               MachineBasicBlock *&BB,
                                               const bool postRA) const {
  return CodasipGenInstrInfo2::ExpandDummiesImpl(MIp, BB, postRA);
}

// Edit this if you are compiling for an environment which can cause
// unpredicable hazards (Linux with I/O interrupts). If you simply return true
// and leave TBB, FBB and Cond unedited, no advanced BB reorganisation will be
// performed
...
bool llvm::CodasipInstrInfo::reverseBranchCondition(
    SmallVectorImpl<MachineOperand> &Cond) const {
  return CodasipGenInstrInfo2::reverseBranchCondition(Cond);
}

unsigned
llvm::CodasipInstrInfo::getEquivalentCondOpcode(ArrayRef<MachineOperand> Cond,
                                                int RegPos) const {
