...
CodasipGenFrameLowering::CodasipGenFrameLowering(const CodasipTargetMachine &tm,
                                                 bool realig)
    : TargetFrameLowering(StackGrowsDown,16,0,16, realig),
      TII(*tm.getInstrInfo()), TRI(TII.getRegisterInfo()) {
...
// Warning: hasCalls is unknown during instruction selection - do not use it
// here note: hasVarSizedObjects is set before selection so it's safe to use
...
bool CodasipGenFrameLowering::hasReservedCallFrame(
...
static inline bool CanForgoProEpi(const MachineFunction &MF) {
  ...
  return !HasCall(MF) && !MF.getFunction().isVarArg() &&
         !MF.getFrameInfo().hasStackObjects();
}

// warning: this assumes that the user doesn't want any "holes"
void CodasipGenFrameLowering::LibSaveRestoreCheck(MachineFunction &MF,
  ...
  // (may or may not include BP, see
  // CodasipGen2RegisterInfo::getCalleeSavedRegs)
...
void CodasipGenFrameLowering::HandleLibSaveRestore(MachineFunction &MF,
                                                   BitVector &SavedRegs) const {
  CodasipFunctionInfo *finfo = MF.getInfo<CodasipFunctionInfo>();
  if (finfo->HasLibSaveRestoreCS()) {
    // anything at all?
...
void CodasipGenFrameLowering::CreateEmergencySpillSlot(MachineFunction &MF,
  ...
  if (MF.getFrameInfo().getNumObjects() > 0) {
    ...
    for (unsigned i = 0; i < MaxFIOperands; ++i) {
      RS->addScavengingFrameIndex(
          MF.getFrameInfo().CreateStackObject(size, align, false));
    }
  }
}

// this is the first method called in the process, edit finfo here if needed
void CodasipGenFrameLowering::determineCalleeSaves(MachineFunction &MF,
                                                   BitVector &SavedRegs,
                                                   RegScavenger *RS) const {
  // call parent first
...
bool CodasipGenFrameLowering::NeedsNoShift(const MachineFunction &MF,
...
int64_t CodasipGenFrameLowering::GetStackShift(const MachineFunction &MF,
                                               unsigned space,
                                               bool noShiftExpl) const {
  const CodasipFunctionInfo *finfo = MF.getInfo<CodasipFunctionInfo>();
...
void CodasipGenFrameLowering::StackAlloc(MachineFunction &MF,
  ...
  MachineInstr::MIFlag miflag(dir ? MachineInstr::FrameDestroy
  ...
  for (unsigned space = 0; space < STACK_COUNT; ++space) {
    ...
    if (finfo->HasLibSaveRestoreCS()) {
      ...
      // note: CSLibShift should be, at this point, a sum of sizes of all cs
      // regs print it for debug purposes
      if (!dir) {
        LLVM_DEBUG(dbgs() << "Function " << MF.getName()
                          << " uses library CS-save/restore which shifts SP by "
                          << CSLibShift << "\n");
      }
...
bool CodasipGenFrameLowering::enableShrinkWrapping(
    const MachineFunction &MF) const {
  const CodasipFunctionInfo *finfo = MF.getInfo<CodasipFunctionInfo>();
  return finfo->GetShrinkWrapEnabled();
}

bool CodasipGenFrameLowering::enableCalleeSaveSkip(
    const MachineFunction &MF) const {
...
void CodasipGenFrameLowering::emitPrologue(MachineFunction &MF,
  ...
  MCCFIInstruction defCfa0 = MCCFIInstruction::createDefCfa(
      nullptr, TRI.getDwarfRegNum(TRI.GetSP(0), 0), 0);
    ...
    if (space == 0) {
      ...
      if (hasCall && !finfo->HasLibSaveRestoreCS()) {
        if (MF.getFunction().hasFnAttribute(Attribute::ShadowCallStack)) {
          ...
          errs() << "There is no Shadow Call Stack pointer register specified! "
                    "Shadow call stack won't be used.\n";
...
// note: certain basic&invariable cfi was left here
void CodasipGenFrameLowering::emitEpilogue(MachineFunction &MF,
                                           MachineBasicBlock &MB) const {
  const CodasipFunctionInfo *finfo = MF.getInfo<CodasipFunctionInfo>();
  unsigned CFIIndex = -1;
  bool hasCall = HasCall(MF);
  ...
  if (finfo->HasLibSaveRestoreCS()) {
    while (II != MB.begin() &&
           std::prev(II)->getFlag(MachineInstr::FrameDestroy)) {
      --II;
    }
  }
  // copy debug info
...
const char *CodasipGenFrameLowering::getCSLibCallName(
    MachineFunction &MF, const std::vector<CalleeSavedInfo> &CSI,
    bool save) const {
  // get least allocatable spilled reg (the last one but before BP(s))
  // see determineCalleeSaves - regs are stored in CSI in same order as
  // getCalleeSavedRegs gives them
  assert(!CSI.empty());
  unsigned topReg = 0;
  bool hasBP = false;
...
return save?"__codasip_libcs_save_rf_gpr_9":"__codasip_libcs_restore_rf_gpr_9";
case Codasip::rf_gpr_18:
return save?"__codasip_libcs_save_rf_gpr_18":"__codasip_libcs_restore_rf_gpr_18";
case Codasip::rf_gpr_19:
return save?"__codasip_libcs_save_rf_gpr_19":"__codasip_libcs_restore_rf_gpr_19";
case Codasip::rf_gpr_20:
return save?"__codasip_libcs_save_rf_gpr_20":"__codasip_libcs_restore_rf_gpr_20";
case Codasip::rf_gpr_21:
return save?"__codasip_libcs_save_rf_gpr_21":"__codasip_libcs_restore_rf_gpr_21";
case Codasip::rf_gpr_22:
return save?"__codasip_libcs_save_rf_gpr_22":"__codasip_libcs_restore_rf_gpr_22";
case Codasip::rf_gpr_23:
return save?"__codasip_libcs_save_rf_gpr_23":"__codasip_libcs_restore_rf_gpr_23";
case Codasip::rf_gpr_24:
return save?"__codasip_libcs_save_rf_gpr_24":"__codasip_libcs_restore_rf_gpr_24";
case Codasip::rf_gpr_25:
return save?"__codasip_libcs_save_rf_gpr_25":"__codasip_libcs_restore_rf_gpr_25";
case Codasip::rf_gpr_26:
return save?"__codasip_libcs_save_rf_gpr_26":"__codasip_libcs_restore_rf_gpr_26";
case Codasip::rf_gpr_27:
return save?"__codasip_libcs_save_rf_gpr_27":"__codasip_libcs_restore_rf_gpr_27";
...
bool CodasipGenFrameLowering::SpillRestoreImpl(
    MachineBasicBlock &MB, MachineBasicBlock::iterator MI,
    const std::vector<CalleeSavedInfo> &CSI, bool spill) const {
  ...
  assert(LibCall);
  auto MIB = BuildMI(MB, MI, DL, TII.get(TII.GetLibCSSpillInstr()),
                     TII.GetLibCSSpillReg());
...
bool CodasipGenFrameLowering::spillCalleeSavedRegisters(
    MachineBasicBlock &MB, MachineBasicBlock::iterator MI,
    const std::vector<CalleeSavedInfo> &CSI,
...
bool CodasipGenFrameLowering::restoreCalleeSavedRegisters(
    MachineBasicBlock &MB, MachineBasicBlock::iterator MI,
...
int CodasipGenFrameLowering::getFrameIndexOffset(const MachineFunction &MF,
  ...
  // fixed objects (we created them in the lowering for passing and receiving of
  // arguments)
  if (FrameIndex < 0) {
...
int CodasipGenFrameLowering::getFrameIndexReference(const MachineFunction &MF,
                                                    int FI,
                                                    unsigned &freg) const {
  const MachineFrameInfo &MFI = MF.getFrameInfo();
...
CodasipGenFrameLowering::eliminateCallFramePseudoInstr(
    MachineFunction &MF, MachineBasicBlock &MB,
    MachineBasicBlock::iterator II) const {
  assert(II->getOpcode() == Codasip::CALLSTART ||
         II->getOpcode() == Codasip::CALLEND);
...
int CodasipGenFrameLowering::getInitialCFAOffset(
    const MachineFunction &MF) const {
...
unsigned CodasipGenFrameLowering::getInitialCFARegister(
    const MachineFunction &MF) const {
  return TRI.getDwarfRegNum(TRI.GetSP(0), 0);
}

void CodasipGenFrameLowering::processFunctionBeforeFrameFinalized(
