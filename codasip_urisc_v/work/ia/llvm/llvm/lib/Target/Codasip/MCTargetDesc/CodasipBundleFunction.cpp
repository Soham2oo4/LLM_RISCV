...
void CodasipBundleFunction::Instruction::DebugPrint(raw_ostream &out) const {
  ...
  out << GetBinary().GetBin(false, false, GetBitWidth()) << "("
      << GetBitWidth() << ")"
      << "\n";
...
/////////////////////////////////////////////////////////////////////////////////
void CodasipBundleFunction::Run(Instructions &outputInstructions,
  ...
  for (bundle<64> &part : parts) {
    ...
    for (size_t bit = 64 - part.size; bit < 64; bit++) {
      ...
      if (fixupIndex == CodasipBundlingBitInfo::UNDEFINED_INDEX) {
        if (fixup) {
          StoreFixup(outputInstruction, fixup, msbValue, lsbValue, msbOut,
                     lsbOut, partOffset, partBytesSize);
        }
      }
      // Current bit has a fixup
      else {
        const MCFixup &currentFixup =
            inputInstructions[bitInfo.GetSlot()].GetFixups()[fixupIndex];
        ...
        // There is a previous fixup
          ...
          // Current fixup is same as previous fixup and continues in previous
          // fixup
          ...
          // Current fixup is different from previous fixup or does not continue
          // in previous fixup
          else {
            StoreFixup(outputInstruction, fixup, msbValue, lsbValue, msbOut,
                       lsbOut, partOffset, partBytesSize);
    ...
    // Print previous fixup on part end, no fixup should cross boundaries of
    // binary parts
    if (fixup) {
      StoreFixup(outputInstruction, fixup, msbValue, lsbValue, msbOut, lsbOut,
...
const CodasipBundleFunction::Instruction &
CodasipBundleFunction::GetInputInstruction(
 ...
 */
void CodasipBundleFunction::StoreFixup(Instruction &instruction,
                                       const MCFixup *&fixup, size_t &msbValue,
                                       size_t &lsbValue, size_t &msbOut,
                                       size_t &lsbOut, const size_t partOffset,
                                       const size_t partBytesSize) {
  // Store current fixup
  instruction.GetFixups().emplace_back(MCFixup::createCodasip(
      partOffset, fixup->getValue(), fixup->getKind(), fixup->getLoc(),
      msbValue, lsbValue, partBytesSize, msbOut, lsbOut,
      fixup->isSignPart() && msbValue == fixup->getMsbValue(),
      fixup->isSigned()));
