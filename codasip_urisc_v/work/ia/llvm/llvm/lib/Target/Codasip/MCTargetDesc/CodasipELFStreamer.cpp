...
public:
  ...
  void emitDirectiveProfiler(const uint64_t Flag, StringRef Block,
                             SMLoc Loc) override;
...
CodasipTargetELFStreamer::CodasipTargetELFStreamer(MCStreamer &S,
                                                   const MCSubtargetInfo &STI)
    : CodasipTargetStreamer(S) {
...
void CodasipTargetELFStreamer::finish() {
  ...
  MCSection *AsTableSection =
      Ctx.getELFSection(".codasip_as", ELF::SHT_CODASIP_AS, 0);
...
void CodasipTargetELFStreamer::emitDirectiveAbsOrg(const uint64_t Address,
                                                   SMLoc Loc) {
  ...
  if (CurrentELFSection->IsAbsoluteAddressSet()) {
    Streamer.getContext().reportError(
        Loc, "Absolute address of a current section is already set.");
    return;
  }
  ...
  if (Address % CurrentELFSection->getAlignment()) {
    Streamer.getContext().reportError(
        Loc, "Absolute address is not aligned on " +
                 std::to_string(CurrentELFSection->getAlignment()) + " bytes.");
    return;
  }
...
void CodasipTargetELFStreamer::emitDirectiveAddressSpace(unsigned Index,
                                                         SMLoc Loc) {
  MCStreamer &Streamer = getStreamer();
  ...
  assert(S->getAssociatedSymbol() == nullptr &&
         "There is some unexpected symbol.");
  MCSectionELF *NewS = Context.getELFSection(
      S->GetSectionBaseName(), S->getType(), S->getFlags(), S->getEntrySize(),
      S->getGroup(), S->getUniqueID(),
      nullptr /*S->getAssociatedSymbol() TODO need this?*/);
...
void CodasipTargetELFStreamer::emitDirectiveProfiler(const uint64_t Flag,
                                                     StringRef Block,
                                                     SMLoc Loc) {
  assert((Flag == CODASIP_PROFILE_FLAG_START ||
  ...
  MCSectionELF *Section =
      Context.getELFSection(CODASIP_PROFILE_INFO_NAME, ELF::SHT_PROGBITS, 0);
...
MCTargetStreamer *
createCodasipObjectTargetStreamer(MCStreamer &S, const MCSubtargetInfo &STI) {
