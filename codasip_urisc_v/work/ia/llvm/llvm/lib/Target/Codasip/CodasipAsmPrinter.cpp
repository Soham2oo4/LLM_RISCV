...
private:
  ...
  void PrintAsmRegisterPair(const MachineInstr *MI, unsigned OpNo,
                            raw_ostream &O) const;
  // for address space
...
CodasipAsmPrinter::CodasipAsmPrinter(TargetMachine &TM,
                                     std::unique_ptr<MCStreamer> MS)
    : AsmPrinter(TM, std::move(MS)) {
  Ctx = NULL;
  hasDebug = false;
  CStream =
      static_cast<CodasipTargetStreamer *>(OutStreamer->getTargetStreamer());
}
...
const MCExpr *CodasipAsmPrinter::LowerSymbolOperand(
    const MachineOperand &MO, const MachineOperand::MachineOperandType MOTy,
    unsigned Offset) {
...
void CodasipAsmPrinter::PrintInstruction(const MachineInstr *MI) {
  ...
  if (!MI->isBundle()) {
    ...
    if (IDesc.isPseudo()) {
      ...
      CodasipInstPrinter Printer(*TM.getMCAsmInfo(), MCII,
...
void CodasipAsmPrinter::PrintAsmRegisterPair(const MachineInstr *MI,
                                             unsigned opNum,
                                             raw_ostream &O) const {
...
void CodasipAsmPrinter::PrintAsmRegOperand(const MachineOperand &MO,
                                           raw_ostream &O) const {
  assert(MO.isReg());
...
// for inline asm
bool CodasipAsmPrinter::PrintAsmOperand(const MachineInstr *MI, unsigned opNum,
...
bool CodasipAsmPrinter::PrintAsmMemoryOperand(const MachineInstr *MI,
                                              unsigned OpNo,
...
// The default behaviour was causing problems on -O0 so it needs to be
// overwritten
bool CodasipAsmPrinter::isBlockOnlyReachableByFallthrough(
...
void CodasipAsmPrinter::EmitGlobalVariable(const GlobalVariable *GV) {
  ...
  //        (=> if it causes SwitchSection then the directives will be in this
  //        order: AS, Switch, GV itself)
  //      - there were some exceptions in AsmPrinter::EmitGlobalVariable
  //        but I don't think they were intended
  // note2: zero-fill is not checked here (Codasip does not support address
  // spaces with those)
...
// Unfortunately, it needs to be this way (I can't just add something and then
// call parent method)
// clang-format off
namespace {
...
void CodasipAsmPrinter::EmitToStreamer(MCStreamer &S, const MCInst &Inst) {
  MCInst CInst;
  bool Res = false;
  if (EnableAsmCompression &&
      TM.getMCSubtargetInfo()->hasFeature(Codasip::FeatureStdExtC))
