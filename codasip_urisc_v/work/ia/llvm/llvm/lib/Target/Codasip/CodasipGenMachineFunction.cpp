...
static const unsigned AlignmentsData[ 1 ] = { 16 };

/// Settings of shrink wrapper - unknown is invalid
enum ShrinkWrapValue { SW_UNKNOWN, SW_OFF, SW_ON };
...
CodasipGenFunctionInfo::CodasipGenFunctionInfo(MachineFunction &MF)
  ...
  if (EnableSaveRestore) {
    ...
    if (!CFL->LibCSSupported()) {
      errs() << "Libcall save/restore of callee saved registers was requested "
...
// Was supposed to reflect calculateFrameObjectOffsets (in
// PrologEpilogInserter.cpp) but it didn't work so it needed to be dumbed down
void CodasipGenFunctionInfo::CalculateFrameObjectOffsets(unsigned space) {
  ...
  for (int i = MFI.getObjectIndexBegin(); i < MFI.getObjectIndexEnd(); ++i) {
    ...
    // checks
    assert(!MFI.isVariableSizedObjectIndex(i) &&
           "doesn't work for variable sized objects");
...
void CodasipGenFunctionInfo::SetShrinkWrapEnabled(bool v) {
  assert(ShrinkWrapEnabled == SW_UNKNOWN &&
         "this was called already for this function");
