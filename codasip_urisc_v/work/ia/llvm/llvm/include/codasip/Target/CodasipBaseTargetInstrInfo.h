...
enum NodeType {
  ...
  HW_LOOP_START, // for IR hwloops to get pseudos through getCodasipInstr
  HW_LOOP_DEC,   // --//--
  Call,          // for C++, don't confuse with target Call
  CODASIP_OP_END
};
}
...
public:
  CodasipBaseInstrInfo(unsigned CFSetupOpcode = ~0u,
  ...
  virtual void insertNoopOnPos(MachineBasicBlock &MBB,
                               MachineBasicBlock::iterator MI,
                               unsigned pos) const {}

  virtual void getCodasipInstr(std::vector<int> &opc, int Operation,
                               MVT Type) const {}

  virtual bool isPredicableBlock(MachineBasicBlock &MBB,
  ...
  // Does the distance from SrcInst position at SrcBlock to Dest block fit the
  // operand OpIdx of Opc?
  // This is useful when JumpInfo isn't available, such as when instruction
  // checked doesn't exist yet.
  virtual bool distanceFits(MachineFunction::iterator SrcBlock,
  ...
  virtual unsigned getBP(unsigned space = 0) const { return 0; }

  virtual unsigned getRA() const { return 0; }

  virtual llvm::SelectionDAG *
  GetCodasipPattern(const TargetMachine &TM, MachineFunction &mf,
                    unsigned icode, std::vector<SDValue> &roots) const {
    return nullptr;
  }

  virtual void getCondBranchOpc(std::vector<int> &Opcs,
