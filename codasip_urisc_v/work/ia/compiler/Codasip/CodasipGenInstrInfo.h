...
/// It's easier to generate this than to read this from instruction information
/// or from GetImmProperties
struct BuiltinInfo {
...
public:
  ...
  void getCodasipInstr(std::vector<int> &Opcs, int Operation,
                       MVT Type) const override;
  void getCondBranchOpc(std::vector<int> &Opcs,
                        ISD::CondCode CC) const override;
  /** Retrieves info about an imm operand.
   * \param opcode  ID of an instruction.
   * \param opnum   Index of an operand. */
  void GetImmProperties(unsigned opCode, unsigned opNum, int &width, bool &sign,
                        int &trunc) const;
  ...
  /** Central implementation of expansion of dummy instructions and emulations
   * with custom insertors.
  ...
  virtual bool ExpandDummiesImpl(MachineInstr *MIp, MachineBasicBlock *&BB,
                                 const bool postRA) const;
  unsigned getRA() const override;
  unsigned getBP(unsigned space) const override;
  llvm::SelectionDAG *
  GetCodasipPattern(const TargetMachine &TM, MachineFunction &mf,
                    unsigned icode, std::vector<SDValue> &roots) const override;
  ...
  void insertNoopOnPos(MachineBasicBlock &MBB, MachineBasicBlock::iterator MI,
                       unsigned pos) const override;
  ...
  virtual bool
  isProfitableToIfCvt(MachineBasicBlock &TMBB, unsigned NumTCycles,
  ...
  virtual bool
  isPredicableBlock(MachineBasicBlock &MBB,
                    SmallVectorImpl<MachineOperand> &Cond) const override;
  virtual bool
  isProfitableToDupForIfCvt(MachineBasicBlock &MBB, unsigned NumCycles,
                            BranchProbability Probability) const override;
  virtual unsigned getEquivalentCondOpcode(ArrayRef<MachineOperand> Cond,
                                           int RegPos) const;
  /// Is lib cs supported?
  ...
  bool distanceFits(MachineFunction::iterator SrcBlock,
                    MachineBasicBlock::instr_iterator SrcInst,
                    MachineBasicBlock *Dest, unsigned Opc,
...
private:
  ...
  MachineBasicBlock::iterator
  identifyPositionInParentBlock(MachineInstr *PredMOSource) const;
  bool getFreePredRegister(MachineInstr *ParentInstr,
                           unsigned *FreePredReg) const;
  bool isThereSuitableCopy(MachineOperand *PredMO) const;
  bool isThereSuitableCondEmul(ArrayRef<MachineOperand> Cond, int RegPos,
                               MachineOperand *PredMO, MachineInstr *PredInstr,
                               bool &InvertJump) const;
  bool emulCondToPredReg(ArrayRef<MachineOperand> Cond, int RegPos,
                         MachineOperand *PredMO, MachineInstr *PredInst) const;
...
inline llvm::SelectionDAG *
CodasipGenInstrInfo2::GetCodasipPattern(const TargetMachine &TM,
