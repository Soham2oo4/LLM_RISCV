 ...
 */
template <int BITS> struct bundle {
  bundle() : size(0) {}

  template <int R_BITS>
  bundle(const bundle<R_BITS> &copy) : value(copy.value), size(copy.size) {}
...
public:
  ...
  /// Large bundles should be split into 64 bit parts to be possible to encode
  /// them in Codasip ELF
   ...
   */
  void Run(Instructions &outputInstructions,
           const Instructions &inputInstructions);
  /**
   *  \brief  Wrapper function around generated bundle function,
   *          it processes input instructions and returns content of result's
   * bundling_int. \param[out] parts Parts of output instructions \param[in]
   * inputInstructions slot instructions of bundle instruction or single
   * instruction
   */
  virtual void Call(Parts &parts,
                    const Instructions &inputInstructions) const = 0;
...
protected:
  ...
  static void InitializeInstruction(bundle<BITS> &instruction,
    ...
    const Instruction &inputInstruction =
        GetInputInstruction(inputInstructions, index);
    instruction.value = CodasipBundlingInt<BITS>(
        inputInstruction.GetBinary().GetBin(false, false,
                                             inputInstruction.GetBitWidth()),
        index);
    instruction.size = inputInstruction.GetBitWidth();
    FillFixupInfo(instruction.value, inputInstruction);
  }
  template <int BITS>
  static void ParcelInstruction(bundle<BITS> &res,
                                const unsigned int parcelBits) {
    ...
    // Do parceling
    // 64bit instruction and 4 parcel bytes: (AAAA BBBB) (CCCC DDDD) => (CCCC
    // DDDD) (AAAA BBBB) 64bit instruction and 2 parcel bytes: (AAAA) (BBBB)
    // (CCCC) (DDDD) => (DDDD) (CCCC) (BBBB) (AAAA)
    bundle<BITS> tmp;
    tmp.value = res.value;
    tmp.size = res.size;
    res.value = 0;
    for (unsigned int i = 0; i < parcelCount; i++) {
      ...
      res.value |= ((tmp.value >> (i * parcelBits)) &
                    codasip::BitMask<uint64_t>(parcelBits));
    }
  }
  template <int BITS>
  static void SplitBundleToParts(Parts &parts, const unsigned int slotBitWidth,
                                 bundle<BITS> &res) {
    // Resulting bundle is already moved to the right of bundle:
    // AAAAAAAABBBBBBBBCCCCCCCC........ => ........AAAAAAAABBBBBBBBCCCCCCCC
    CHECK_CONDITION((res.size % slotBitWidth) == 0)
        << "Output bundle (" << res.size
        << ") is not divisible by slot bit-width " << slotBitWidth << ".";
    uint32_t readBits = res.size;
    const unsigned int slotCount =
        res.size / slotBitWidth; // Bundling could remove some slots
    for (unsigned int i = 0; i < slotCount; i++) {
      bundle<64> dst;
      dst.value = (res.value >> (readBits - slotBitWidth)) &
                  codasip::BitMask<uint64_t>(slotBitWidth);
      dst.size = slotBitWidth;
      readBits -= slotBitWidth;
      // Pass bit contents of output instructions to the parts, endian does not
      // affect slots
...
private:
  template <int BITS>
  static void FillFixupInfo(CodasipBundlingInt<BITS> &value,
                            const Instruction &inputInstruction) {
  ...
  GetInputInstruction(const Instructions &inputInstructions,
                      const size_t index);
  static void StoreFixup(Instruction &instruction, const MCFixup *&fixup,
                         size_t &msbValue, size_t &lsbValue, size_t &msbOut,
                         size_t &lsbOut, const size_t partOffset,
                         const size_t partBytesSize);
};

////////////////////////////////////////////////////////////////////////////////////////////////////
inline CodasipBundleFunction::Instruction::Instruction(const uint64_t bw)
    : m_Binary(0), m_BitWidth(bw) {}

inline const codasip::MaxUint &
CodasipBundleFunction::Instruction::GetBinary() const {
  return m_Binary;
}
...
inline CodasipBundleFunction::Instruction::Fixups &
CodasipBundleFunction::Instruction::GetFixups() {
  return m_Fixups;
}

inline const CodasipBundleFunction::Instruction::Fixups &
CodasipBundleFunction::Instruction::GetFixups() const {
  return m_Fixups;
}

inline void
CodasipBundleFunction::Instruction::SetBinary(const codasip::MaxUint &binary) {
  m_Binary = binary;
}
