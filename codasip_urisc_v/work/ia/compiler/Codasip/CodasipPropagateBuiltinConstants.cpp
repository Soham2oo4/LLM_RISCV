...
struct PropagateBuiltinConstants : public FunctionPass {
  ...
  PropagateBuiltinConstants(CodasipTargetMachine &tm)
      : FunctionPass(ID), CTM(tm) {}

  llvm::StringRef getPassName() const override {
    return "Propagate builtin constants";
  }
...
bool PropagateBuiltinConstants::handleCodasipBuiltin(IntrinsicInst *II) {
  ...
  for (auto arg : imms) {
    ...
    CastInst *CII = CastInst::CreateIntegerCast(
        CI,
        // clang-format off
...
ConstantInt *PropagateBuiltinConstants::getConstantForValue(Value *V,
                                                            bool &sign) {
...
FunctionPass *
llvm::createCodasipPropagateBuiltinConstantsPass(CodasipTargetMachine &tm) {
