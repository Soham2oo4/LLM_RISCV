...
static cl::opt<bool>
    UseLongCalls("long-calls", cl::init(false),
                 cl::desc("Pick the longest call where available"));

static cl::opt<bool>
    UseShortCalls("no-long-calls", cl::init(false),
...
void CodasipGenDAGToDAGISel::PreprocessISelDAG() {
    ...
    if (N->getOpcode() == ISD::ADDRSPACECAST) {
      ...
      if (op->getOpcode() == ISD::FrameIndex) {
        ...
        if (MFI.getStackID(FI)) {
          assert(MFI.getStackID(FI) == space &&
                 "unexpected address space conflict");
        } else {
        ...
        errs() << "It's forbidden to use address space casts. They are "
                  "reserved for internal usage only (marking of multi-stack "
                  "local variables).\n";
        errs() << "This could be caused by casting pointer from one address "
                  "space to pointer from another.\n";
        exit(1);
      }
      // replace
  ...
  for (SelectionDAG::allnodes_iterator I = CurDAG->allnodes_begin();
    ...
    if (N->getOpcode() == ISD::FrameIndex) {
      ...
      //     original: (add fi,imm)   preprocessed: (add (add tfi,0),imm)
      //     desired: (add tfi,0) & (add tfi,imm)
...
void CodasipGenDAGToDAGISel::adjustCallLengths() {
  ...
  while (Position != CurDAG->allnodes_end()) {
    ...
    if (OrigDesc.isTerminator())
      ...
      NewOpc =
          (LongCall) ? CII->getLongCallOpcode() : CII->getShortCallOpcode();
    ...
    if (NewOpc) {
      ...
      LLVM_DEBUG(dbgs() << "Adjusting call length: opc #"
                        << N->getMachineOpcode() << " --> opc #" << NewOpc
                        << "\n";);
...
bool CodasipGenDAGToDAGISel::SelectOffsettable(const SDValue &Op,
  ...
  if (op1.getOpcode() != ISD::TargetFrameIndex)
    return true;
  if (op2.getOpcode() != ISD::Constant)
    return true;
  ConstantSDNode *imm = static_cast<ConstantSDNode *>(op2.getNode());
  SDValue timm = CurDAG->getTargetConstant(*imm->getConstantIntValue(), imm,
                                           imm->getValueType(0));
  // all is ok
...
// select an address
bool CodasipGenDAGToDAGISel::SelectInlineAsmMemoryOperand(
  ...
  // unsupported
  default:
    return SelectionDAGISel::SelectInlineAsmMemoryOperand(Op, ConstraintCode,
                                                          OutOps);
  }
  // all ok
  return false;
...
bool CodasipGenDAGToDAGISel::CheckSymbol(SDNode *n, int width, bool sign,
                                         int trunc) const {
...
bool CodasipGenDAGToDAGISel::CheckImm(int64_t Imm, int width, bool sign,
                                      int trunc) const {
...
bool CodasipGenDAGToDAGISel::CheckLoadStore(const MemSDNode *n, int align,
                                            int space) const {
  assert(n);
  ...
  // custom handling
    ...
    const FixedStackPseudoSourceValue *fpsv =
        dyn_cast<const FixedStackPseudoSourceValue>(psv);
...
void CodasipGenDAGToDAGISel::LowerMoiPatterns(SelectionDAG *DAG) {
  ...
  for (int i = 0; i < MOI_GROUP_MAP_SIZE; ++i) {
    SelectionDAG *Pattern =
        GetCodasipPattern(TM, *MF, InstrGrpMap[i][0], roots);
    if (roots.empty()) {
      continue;
    }
