...
void CodasipGenPassConfig::addPreEmitPass() {
  ...
  // note: and this is actually fourth dummy expansion, dummies can be inserted
  // even after the third one
  addPass(createCodasipCallCompletionPass(tm));
  // this must be last
...
void CodasipGenPassConfig::addPreEmitPass2() {
  ...
  // For example fixes cases when unwind resume is generated after epilogue of
  // function
...
public:
  ...
  StringRef getPassName() const { return "CodasipPreISel"; }
};
} // namespace
      ...
      for (BasicBlock::iterator BI = FI->begin(); BI != FI->end(); ++BI) {
        ...
        if (BI->getOpcode() == Instruction::VAArg) {
          ...
          if (et.isSimple() && et == MVT::Other) {
            errs() << "There's an invalid type in va_arg. This isn't allowed. "
