...
struct Expander : public MachineFunctionPass {
  Expander(CodasipTargetMachine &TM)
      : MachineFunctionPass(ID), CII(TM.getInstrInfo()) {}
  StringRef getPassName() const {
    return "Codasip dummy instructions expander";
  }
...
public:
  AsmPairer(CodasipTargetMachine &TM)
      : MachineFunctionPass(ID), CII(TM.getInstrInfo()) {}
  StringRef getPassName() const {
    return "Codasip inline asm reg pairs handler";
  }
  bool runOnMachineFunction(MachineFunction &MF);
...
public:
  ...
  StringRef getPassName() const {
    return "Codasip inline asm fast reg pre-allocator";
  }
  bool runOnMachineFunction(MachineFunction &MF);
  void getAnalysisUsage(AnalysisUsage &AU) const override;
  static char ID;
...
bool Expander::runOnMachineFunction(MachineFunction &F) {
  bool changed = false;
  // aux function
  auto AuxHandle = [this, &changed](MachineBasicBlock::iterator I,
...
typedef std::map<unsigned, std::vector<const MachineOperand *>>
    InlineAsmRegPairs;
  ...
  for (const MachineOperand &MO : MI.operands()) {
    ...
    case S_BEGIN:
      ...
      // unexpected reg
        ...
        MI.emitError(
            "Faulty usage of 'p' in inline asm (long data used without 'p')");
        return fret;
      }
    ...
    case S_OP:
      ...
      if (ret.count(ind)) {
        ...
        if (!MO.isReg()) {
          MI.emitError(
              "Faulty usage of 'p' in inline asm (was used with wrong data)");
          return fret;
        }
    ...
    case S_REG:
      ...
      if (!MO.isReg()) {
        MI.emitError(
            "Faulty usage of 'p' in inline asm (was used with short data)");
        return fret;
      }
      // store
      regOps.push_back(&MO);
...
// note: greedy allocator doesn't always listen to getRegAllocationHints (even
// if hard hint)
...
FunctionPass *
llvm::createCodasipInlineAsmRegPairsPass(CodasipTargetMachine &tm) {
  return new AsmPairer(tm);
}

FunctionPass *
llvm::createCodasipInlineAsmFastRegAlloc(CodasipTargetMachine &tm) {
