# 1 "/tools/toolchain/newlib/runtime/mcount.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 325 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "/tools/toolchain/newlib/runtime/mcount.c" 2
# 46 "/tools/toolchain/newlib/runtime/mcount.c"
...
# 47 "/tools/toolchain/newlib/runtime/mcount.c" 2
...
_Noreturn void
 quick_exit(int);
# 48 "/tools/toolchain/newlib/runtime/mcount.c" 2
...
# 49 "/tools/toolchain/newlib/runtime/mcount.c" 2
...
# 50 "/tools/toolchain/newlib/runtime/mcount.c" 2
...
# 37 "/tools/toolchain/newlib/newlib/libc/include/time.h" 3
struct tm
{
  int tm_sec;
  int tm_min;
...
# 51 "/tools/toolchain/newlib/runtime/mcount.c" 2
# 115 "/tools/toolchain/newlib/runtime/mcount.c"
struct tostruct
{
...
void __attribute__((no_instrument_function)) _mcount_internal (unsigned long frompc, unsigned long selfpc);
void __attribute__((no_instrument_function)) _monstartup (unsigned long, unsigned long);
void __attribute__((no_instrument_function)) _mcleanup (void);

static struct gmonparam _gmonparam;

void
_mcount_internal (unsigned long frompc, unsigned long selfpc)
{
...
_monstartup (unsigned long lowpc, unsigned long highpc)
{
  ...
  p->lowpc = (((lowpc) / (2 * sizeof (* p->kcount))) * (2 * sizeof (* p->kcount)));
  p->highpc = ((((highpc) + (2 * sizeof (* p->kcount)) - 1) / (2 * sizeof (* p->kcount))) * (2 * sizeof (* p->kcount)));
  p->textsize = p->highpc - p->lowpc;
  p->kcountsize = ((((p->textsize / 2) + (sizeof (*p->froms)) - 1) / (sizeof (*p->froms))) * (sizeof (*p->froms)));
  p->hashfraction = 2;
  p->log_hashfraction = -1;
  p->log_hashfraction = ffs (p->hashfraction * sizeof (*p->froms)) - 1;
  p->fromssize = p->textsize / 2;
  p->tolimit = p->textsize * 3 / 100;

  if (p->tolimit < 50)
    p->tolimit = 50;
  else if (p->tolimit > (1L << 20))
    p->tolimit = (1L << 20);
...
static void
write_call_graph (FILE *fd)
{


  struct gmon_cg_arc_record raw_arc[32]
  ...
  for (nfilled = 0; nfilled < 32; ++ nfilled)
    ...
    to_index = _gmonparam.tos[to_index].link)
 {
   ...
   uint32_t tmp_count = _gmonparam.tos[to_index].count;

   memcpy (arc->from_pc, & frompc, sizeof (arc->from_pc));
   memcpy (arc->self_pc, & _gmonparam.tos[to_index].selfpc, sizeof (arc->self_pc));
   memcpy (arc->count, & tmp_count, sizeof (arc->count));

   if (++ nfilled == 32)
     {
       fwrite (raw_arc, sizeof raw_arc, 1, fd);
       nfilled = 0;
     }
 }
    }
  ...
  if (nfilled > 0)
    fwrite (raw_arc, sizeof (raw_arc[0]), nfilled, fd);
}
...
# 392 "/tools/toolchain/newlib/runtime/mcount.c" 2
...
write_gmon (void)
{
  ...
  memcpy (ghdr.cookie, "gmon", sizeof (ghdr.cookie));
  * (unsigned long *) ghdr.version = 1;
  fwrite (& ghdr, sizeof (ghdr), 1, fd);
...
_mcleanup (void)
{
  ...
  if (_gmonparam.tos != ((void*)0))
    {
      free (_gmonparam.tos);
      _gmonparam.tos = ((void*)0);
    }
}
